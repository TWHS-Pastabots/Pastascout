  ...
  return (
    <div className="mx-auto flex max-w-md flex-col gap-6 px-4 py-8">
      ...
      <section className="rounded-xl border border-slate-800 bg-slate-900 p-4">
        ...
        <input
          className="mt-3 w-full rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-slate-100 focus:border-sky-500 focus:outline-none"
        ...
        <button
          onClick={testAndSave}
          className="mt-3 w-full rounded-lg bg-sky-600 px-4 py-2 font-medium text-white hover:bg-sky-500"
        >
          Save & test connection
        </button>
        {status === "checking" && <p className="mt-2 text-sm text-slate-400">Checking…</p>}
        {status === "ok" && <p className="mt-2 text-sm text-emerald-400">Connected.</p>}
        {status === "fail" && (
